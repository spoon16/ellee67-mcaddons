 ...
 */
export function blastDamage(power, distance, exposure, difficulty = "Normal") {
  ...
  switch (String(difficulty).toLowerCase()) {
    ...
    case "easy": return Math.min(raw, Math.floor(raw / 2 + 1));
    case "hard": return Math.floor(raw * 1.5);
    default: return raw;
  }
}

/** Twelve rays sample a player's body; an unreadable ray counts as covered. */
export function exposureAt(dimension, origin, feet, headY) {
  const top = Math.max(feet.y + 0.2, headY - 0.05);
  const heights = [feet.y + 0.1, (feet.y + top) / 2, top];
  let visible = 0;
  for (const dx of [-0.23, 0.23]) for (const dz of [-0.23, 0.23]) for (const y of heights) {
    const delta = { x: feet.x + dx - origin.x, y: y - origin.y, z: feet.z + dz - origin.z };
    const length = Math.hypot(delta.x, delta.y, delta.z);
    if (length < 0.05) { visible++; continue; }
    ...
    try {
      ...
      if (!hit) visible++;
    } catch { /* An unloaded/unreadable ray is not proof of exposure. */ }
  }
  return visible / 12;
}
 ...
 */
export function createCreeperHandler({ world, system, GameMode, EntityDamageCause, warn = console.warn }) {
  const processed = new Map();
  return function onExplosion(event) {
    const source = event.source;
    if (!source || source.typeId !== "minecraft:creeper") return;
    ...
    let dimension, origin, power, snapshots = [];
    try {
      dimension = source.dimension;
      const feet = source.location;
      origin = { x: feet.x, y: feet.y + 0.8, z: feet.z };
      power = source.getComponent("minecraft:is_charged") ? 6 : 3;
    ...
    system.run(() => {
      ...
      try { if (source.isValid) source.remove(); }
      catch (error) { warn(`[67 Creeper Mod] Detonator cleanup: ${error}`); }
      if (!dimension || !origin) return;
      try { dimension.spawnParticle("minecraft:huge_explosion_emitter", origin); }
      catch (error) { warn(`[67 Creeper Mod] Visual effect: ${error}`); }
      try { dimension.playSound("random.explode", origin, { volume: 1, pitch: 1 }); }
      catch (error) { warn(`[67 Creeper Mod] Sound effect: ${error}`); }
      for (const hit of snapshots) {
        try {
          ...
          const mode = player.getGameMode();
          if (mode !== GameMode.Survival && mode !== GameMode.Adventure) continue;
          const hurt = player.applyDamage(hit.damage, { cause: EntityDamageCause.entityExplosion });
          if (hurt) player.applyKnockback(hit.horizontal, hit.vertical);
        } catch (error) { warn(`[67 Creeper Mod] Player damage: ${error}`); }
      }
    });
  };
}
