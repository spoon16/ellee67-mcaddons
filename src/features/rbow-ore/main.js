...
export function registerToolComponent(registry) {
      ...
      try {
        ...
        if (action.state) replacement=target.permutation.withState(action.state,action.value);
          ...
          if (action.keepStates) for (const [name,value] of Object.entries(original.getAllStates())) {
            if (Object.prototype.hasOwnProperty.call(replacement.getAllStates(),name)) replacement=replacement.withState(name,value);
          }
        }
