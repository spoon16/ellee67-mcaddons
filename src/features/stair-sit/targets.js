...
import { distanceSquared } from "./stairs.js";
import { eligible, emptyHands, getBlock, hasHeadroom, nearbyStair, readStair, removeSeat, valid } from "./seats.js";
...
function* discoveryPositions(center, preferred) {
...
export class InteractionTargets {
  constructor(world, system, seats) {
    this.world = world;
    this.system = system;
    this.seats = seats;
  ...
  enabled(player) {
    try { return player.getDynamicProperty(CONFIG.buttonProperty) !== false && !player.hasTag(CONFIG.buttonDisabledTag); }
    catch { return false; }
  }
  canRequest(player) {
    try {
      const seat = this.seats.get(player.id);
      return this.enabled(player) && emptyHands(player) &&
        !this.seats.onCooldown(player.id) && !eligible(player, seat?.seat);
    } catch { return false; }
  }
  get(entityId) { return this.byEntity.get(entityId); }
  remove(key) {
  ...
  suppress(key, ticks = CONFIG.targetSuppressTicks) {
  ...
  candidates(player, preferredStair) {
    ...
    for (const pos of discoveryPositions(center, preferred)) {
      if (distanceSquared(origin, { x: pos.x + 0.5, y: pos.y + 0.5, z: pos.z + 0.5 }) > (CONFIG.reach + 0.5) ** 2) continue;
      const stair = readStair(getBlock(player.dimension, pos));
      if (!stair || this.seats.byBlock.has(stair.key) || (this.suppressed.get(stair.key) ?? -1) > this.system.currentTick) continue;
    ...
    for (const player of this.world.getAllPlayers()) {
      ...
      try {
        for (const candidate of this.candidates(player)) {
          if (!wanted.has(candidate.stair.key) && wanted.size < CONFIG.maxTargetsTotal) wanted.set(candidate.stair.key, candidate);
        }
      } catch { /* A disconnected player or unloaded chunk cannot request targets. */ }
    }
    for (const [key, record] of this.byBlock) {
      const request = wanted.get(key);
      if (!request || !valid(record.entity) || record.entity.dimension.id !== record.dimension.id ||
          distanceSquared(record.entity.location, record.anchor) > 0.01 ||
          request.stair.fingerprint !== record.stair.fingerprint) this.remove(key);
    }
   ...
   */
  refreshForPlayer(player, vacatedStair) {
    this.pruneOccupied();
    ...
    } catch { /* Target discovery must not undo an already successful sit/move. */ }
  }
  /** Shared, bounded, idempotent target creation and renewal. */
  ensureTarget({ stair, dimension }, tick = this.system.currentTick) {
    const key = stair.key;
    if (tick < this.pausedUntil || this.seats.byBlock.has(key) || (this.suppressed.get(key) ?? -1) > tick) return;
    let record = this.byBlock.get(key);
    try {
      if (record && (!valid(record.entity) || record.entity.dimension.id !== dimension.id ||
          distanceSquared(record.entity.location, record.anchor) > 0.01 ||
  ...
  sweep(clearAll = false) {
    ...
    for (const id of ["overworld", "nether", "the_end"]) {
      try { const d = this.world.getDimension(id); dimensions.set(d.id, d); } catch { /* Unavailable. */ }
    }
    ...
    for (const dimension of dimensions.values()) {
      try {
        for (const entity of dimension.getEntities({ type: CONFIG.targetEntityId })) {
          if (!this.byEntity.has(entity.id)) { removeSeat(entity); count++; }
        }
