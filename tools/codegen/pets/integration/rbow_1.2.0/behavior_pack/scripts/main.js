import { world, system, ItemStack, BlockPermutation, EquipmentSlot, GameMode } from '@minecraft/server';
import { NS, BLOCKS, GEAR, miningDrop, durabilityLoss, enchantment, toolAction } from './rules.js';
...
system.beforeEvents.startup.subscribe(event => {
  event.itemComponentRegistry.registerCustomComponent('elleedog:rbow_tool', {
    ...
    onMineBlock(event) {
      ...
      } catch(error) { warnOnce('Tool mining durability', error); }
    },
    onBeforeDurabilityDamage(event) {
      const type=event.itemStack?.typeId;
      if (GEAR.has(type)) event.durabilityDamage = [NS+'rbow_sword',NS+'rbow_hoe'].includes(type) ? 1 : 2;
    }
  });
});
...
world.afterEvents.playerBreakBlock.subscribe(event => {
  try {
    ...
    for(let remaining=drop.amount;remaining>0;remaining-=64)
      event.dimension.spawnItem(new ItemStack(drop.typeId,Math.min(64,remaining)),{x:location.x+.5,y:location.y+.5,z:location.z+.5});
  } catch(error) { warnOnce('Rbow mining loot',error); }
});
...
world.beforeEvents.playerInteractWithBlock.subscribe(event => {
  try {
    const player=event.player, tool=event.itemStack;
    ...
    const block=event.block, original=block.permutation;
    const above=block.above();
    const action=toolAction(tool.typeId,block.typeId,!!above?.isAir,event.blockFace);
    if (!action) return;
    if (action.state && original.getState(action.state)===action.value) return;
    event.cancel=true;
    const key=player.id;
    if (pending.has(key)) return;
    pending.add(key);
    const dimension=block.dimension, position={...block.location}, slot=player.selectedSlotIndex;
    system.run(()=>{
      try {
        ...
        if (!target || !target.permutation.matches(original.type.id,original.getAllStates())) return;
        if (!action.state && [NS+'rbow_hoe',NS+'rbow_shovel'].includes(tool.typeId) && !target.above()?.isAir) return;
        let replacement;
        if (action.state) replacement=target.permutation.withState(action.state,action.value);
        else {
          replacement=BlockPermutation.resolve(action.block);
          if (action.keepStates) for (const [name,value] of Object.entries(original.getAllStates())) {
            if (Object.hasOwn(replacement.getAllStates(),name)) replacement=replacement.withState(name,value);
          }
        }
        target.setPermutation(replacement);
        dimension.playSound(action.sound,position);
        if (action.extra && world.gameRules.doTileDrops) dimension.spawnItem(new ItemStack(action.extra),{x:position.x+.5,y:position.y+1,z:position.z+.5});
        consumeDurability(player,tool.typeId,1);
      } catch(error) { warnOnce('Rbow tool use',error); }
      finally { pending.delete(key); }
    });
  } catch(error) { warnOnce('Rbow tool interaction',error); }
});
...
system.afterEvents.scriptEventReceive.subscribe(event=>{
  ...
  for(const type of ['sword','pickaxe','axe','shovel','hoe','spear','helmet','chestplate','leggings','boots']) {
    try {
    ...
    } catch(error) { lines.push(`${type}: ${String(error)}`); }
  }
  for (const id of BLOCKS) {
    try {
      const block = BlockPermutation.resolve(id);
      lines.push(`${block.type.id === id ? 'OK' : 'FAIL'} block registration: ${id}`);
    } catch(error) { lines.push(`FAIL block ${id}: ${String(error)}`); }
  }
  lines.push('Native drops enabled; no scripted explosion or item-damage interception.');
  lines.push(`Recipe discovery: recipesUnlock=${world.gameRules.recipesUnlock}; showRecipeMessages=${world.gameRules.showRecipeMessages}`);
  lines.push('This checks registrations/components, not client icons or manual equip/place behavior.');
  const output=lines.join('\n');
  if(event.sourceEntity?.typeId==='minecraft:player') event.sourceEntity.sendMessage(output);
